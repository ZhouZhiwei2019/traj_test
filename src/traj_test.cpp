...
public:
    TrajectoryPublisher()
    {
        ...
        ros::param::get("~axis_vmax", axis_vmax_);
    ...
    void publishTrajectory(const ros::TimerEvent&)
    {
        ...
        {
            // 生成S形轨迹：简单的S形曲线
            target.position.x = axis_vmax_ * radius_ * sin(t);  // S形曲线X坐标
            target.position.y = axis_vmax_ * t;                 // Y坐标随时间变化
            target.position.z = height_;                        // 固定高度
            target.yaw        = 0.0;

            // 计算S形轨迹的速度
            double v_x     = axis_vmax_ * radius_ * cos(t);
            double v_y     = axis_vmax_;  // y轴速度是axis_vmax_
            double v_z     = 0.0;
        ...
        {
            // 生成O形轨迹：圆形轨迹
            target.position.x = axis_vmax_ * radius_ * cos(t);  // 圆形曲线X坐标
            target.position.y = axis_vmax_ * radius_ * sin(t);  // 圆形曲线Y坐标
            target.position.z = height_;                        // 固定高度
            target.yaw        = 0.0;

            // 计算O形轨迹的速度
            double v_x     = axis_vmax_ * -radius_ * sin(t);
            double v_y     = axis_vmax_ * radius_ * cos(t);
            double v_z     = 0.0;
            double v_total = sqrt(v_x * v_x + v_y * v_y + v_z * v_z);
